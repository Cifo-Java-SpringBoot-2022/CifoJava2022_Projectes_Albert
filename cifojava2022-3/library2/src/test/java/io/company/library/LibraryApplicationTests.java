...
class LibraryApplicationTests {
	...
	@Test
	void deleteBookById(){
		// FIRST, CREATE A BOOK PRIOR TO DELETE IT
		Book book_n = new Book("BOOK TO DELETE BY ID",  250, 2000, "TEST-5-TO_DELETE-5555");
		//to repo-DB via bookService
		bookService.createBook(book_n);
		//Create an Optional <Book> if find it in the DB by title
		Optional<Book> bookFound = bookService.findBookByTitle("BOOK TO DELETE BY ID");
		if (bookFound.isPresent()){
			//Get the Object book
			Book bookPresent = bookFound.get();
			//Get the Id from the Object book
			long id_book = bookPresent.getBookId();
			//Delete book by id id_book
			bookService.deleteBookById(id_book);
			//Check if the Book by the Id is in the DB
			assertEquals(bookService.findBookById(id_book), Optional.empty());
		}
		//assertEquals(bookRepository.findBookByTitle("BOOK TO DELETE"), null);

	}
	@Test
	void deleteBookByTitle(){
		// FIRST WE CREATE A BOOK PRIOR TO DELETED
		Book book_n = new Book("BOOK TO DELETE BY TITLE",  250, 2000, "TEST-5-TO_DELETE-5555");
		//to repo-DB via bookService
		bookService.createBook(book_n);
		//Create an Optional <Book> if find it in the DB by title
		bookService.deleteBookByTitle("BOOK TO DELETE BY TITLE");

		//Create an Optional <Book> if find it in the DB by title
		Optional<Book> bookFound = bookService.findBookByTitle("BOOK TO DELETE BY TITLE");

		//assertEquals(bookFound.isPresent(), false);
